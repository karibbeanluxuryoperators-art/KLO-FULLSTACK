  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          <h2 className="text-4xl font-serif text-white uppercase tracking-tighter mb-2">{t.title}</h2>
          <p className="text-luxury-cream/40 font-mono text-[10px] uppercase tracking-[0.3em]">{t.subtitle}</p>
        </div>
        <div className="flex gap-4 w-full md:w-auto">
          ...
          <button 
            ...
            className="px-8 py-4 bg-gold text-luxury-black rounded-2xl font-bold uppercase tracking-widest text-[10px] flex items-center gap-3 hover:bg-white transition-all shrink-0"
          >
              ...
              <div className="flex items-center gap-6">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <span className={`px-3 py-1 text-[10px] uppercase tracking-widest rounded-full border font-bold ${
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div>
                    <span className="text-[10px] text-luxury-cream/30 uppercase block mb-1">{t.dietary}</span>
                  ...
                  <div>
                    <span className="text-[10px] text-luxury-cream/30 uppercase block mb-1">{t.beverages}</span>
                    <div className="flex flex-wrap gap-2">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 bg-white/5 rounded-2xl border border-white/10">
                    <span className="text-[10px] text-luxury-cream/30 uppercase block">{t.experiences}</span>
                    <span className="text-xl font-serif">{profile.pastExperiences}</span>
                  </div>
                  <div className="p-4 bg-white/5 rounded-2xl border border-white/10">
                    <span className="text-[10px] text-luxury-cream/30 uppercase block">{t.loyalty}</span>
                    <span className="text-xl font-serif">{profile.loyaltyPoints}</span>
                  </div>
                </div>
                <div>
                  <span className="text-[10px] text-luxury-cream/30 uppercase block mb-2">{t.interests}</span>
                  <div className="flex flex-wrap gap-2">
            ...
            >
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-2xl font-serif text-white">{t.addClient}</h3>
                    <p className="text-[10px] text-luxury-cream/40 uppercase tracking-widest">New Guest Registration</p>
                  </div>
                </div>
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] uppercase tracking-widest text-luxury-cream/40 ml-4">{t.name}</label>
                    <input 
                      type="text" 
                      required
                      value={newClient.name || ''}
                      onChange={e => setNewClient({...newClient, name: e.target.value})}
                      className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 focus:outline-none focus:border-gold/50 transition-all text-sm"
                      placeholder="e.g. Alexander Vianco"
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] uppercase tracking-widest text-luxury-cream/40 ml-4">{t.tier}</label>
                    <select 
                      value={newClient.tier}
                      onChange={e => setNewClient({...newClient, tier: e.target.value as any})}
                      className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 focus:outline-none focus:border-gold/50 transition-all text-sm appearance-none"
                    >
                ...
                <div className="space-y-4">
                  <h4 className="text-[10px] uppercase tracking-widest text-gold ml-4">{t.preferences}</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase tracking-widest text-luxury-cream/40 ml-4">{t.dietary}</label>
                      <input 
                        type="text" 
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        {newClient.preferences?.dietary.map((d, i) => (
                          <span key={i} className="text-[10px] px-2 py-1 bg-gold/10 text-gold rounded-md border border-gold/20 flex items-center gap-2">
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase tracking-widest text-luxury-cream/40 ml-4">{t.beverages}</label>
                      <input 
                        type="text" 
                        placeholder="Comma separated..."
                        onKeyDown={e => {
                ...
                <div className="pt-6 flex gap-4">
                  <button 
                    type="button"
                    onClick={() => setIsAddModalOpen(false)}
                    className="flex-1 py-4 border border-white/10 rounded-2xl text-luxury-cream/40 font-bold uppercase tracking-widest text-[10px] hover:bg-white/5 transition-all"
                  >
                    {t.cancel}
                  </button>
                  <button 
                    type="submit"
                    className="flex-1 py-4 bg-gold text-luxury-black rounded-2xl font-bold uppercase tracking-widest text-[10px] hover:bg-white transition-all flex items-center justify-center gap-3"
                  >
